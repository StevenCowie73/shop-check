...
const VOICE_DISCLOSURE =
  "Thanks for calling ColdenJames. If we miss your call, we'll text you back " +
...
/* Heard only by whoever picks up the forwarded call on Steven's cell,
   before the caller is put through. Pressing 1 takes the call; anything
   else — including voicemail, which presses nothing — leaves it missed. */
const SCREEN_PROMPT = 'ColdenJames call. Press 1 to take it.';
...
/* A text to the business number, passed on to Steven's cell. This is the
   exact shape of sample 3 in the approved texting campaign. `from` is the
   sender already formatted as +1 318-555-0142. */
const forwardedText = (from, message) =>
  'ColdenJames: new text from ' + from + ': "' + message + '" Reply STOP to opt out.';

/* What Steven is told when one of his replies does not go out. One short
   line each, sent only on failure; a reply that goes out gets no answer. */
const NOT_SENT = {
  notAllowed: "Not sent: that number hasn't contacted ColdenJames.",
  nobody: 'Not sent: nobody has called or texted ColdenJames yet.',
  empty: 'Not sent: there was no message after the number.',
  optedOut: 'Not sent: that number has replied STOP.',
  refused: 'Not sent: Twilio refused the message.',
  unknown: "Not sent: couldn't check the call and text log. Try again."
};

module.exports = {
  VOICE_DISCLOSURE, MISSED_CALL_TEXT, LETTER_SMS_LINE,
  MISSED_CALL_CALLBACK, MISSED_CALL_TEXT_SENT, SCREEN_PROMPT, SPOKEN,
  forwardedText, NOT_SENT
};

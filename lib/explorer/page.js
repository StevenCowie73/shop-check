...
const { mapFit } = require('./map-fit.js');
const MAPS_JS = 'https://maps.googleapis.com/maps/api/js';

/* Where the map opens when the link names no area: the area the letters
   go to first. "All areas" is one tap away in the picker. */
const DEFAULT_MAP_AREA = 'bossier-caddo';
...
(function () {
  ...
  var DEFAULT_MAP_AREA = __DEFAULT_MAP_AREA__;
  var pw = '';
  ...
  /* ---------- Map ---------- */
  /* The map opens on DEFAULT_MAP_AREA when the link names no area, so
     "All areas" needs a value of its own to stay chosen. */
  function mapArea(q) {
    if (q.has('area')) return q.get('area') === 'all' ? '' : q.get('area');
    return boot.areas.some(function (a) { return a.id === DEFAULT_MAP_AREA; }) ? DEFAULT_MAP_AREA : '';
  }
  function areaOptions(sel) {
    return '<option value="all">All areas</option>' + boot.areas.map(function (a) {
  ...
  function mapView(q) {
    ...
    document.getElementById('marea').onchange = function () { location.hash = '#/map?area=' + encodeURIComponent(this.value); };
    var el = document.getElementById('map');
    ...
    Promise.all([loadMaps(boot.mapsKey), api('pins', area ? { area: area } : {})]).then(function (r) {
      ...
      /* Open on the area's own businesses, not every far-off mailing address. */
      var fit = mapFit(j.pins, area);
      if (fit.center) { map.setCenter(fit.center); map.setZoom(fit.zoom); }
      else {
  ...
  function businessView(id) {
    ...
    api('business', { id: id }).then(function (j) {
      ...
      var letter = b.letter ? (b.letter.state === 'sent' ? 'Sent ' + day(b.letter.sentAt) : b.letter.state === 'mock' ? 'Mock letter — never to be sent' : b.letter.state === 'approved' ? 'Approved, not sent yet' : 'Drafted, not sent') : 'No letter yet';
      view.innerHTML = '<a class="back" href="javascript:history.back()">← Back</a>' +
        ...
        '<h2>Licence</h2><div class="card"><dl class="facts">' +
          ...
          /* Letter codes never contain an L (lib/refs.js), so an id that
             starts with one is a licence number: no code was issued. */
          '<dt>Reference</dt><dd>' + (/^L/.test(b.id) ? '— no letter code' : esc(b.id)) + '</dd>' +
          (b.rank ? '<dt>Rank</dt><dd>' + esc(b.rank) + (b.selected ? ' (picked)' : '') + '</dd>' : '') +
        '</dl></div>' +
  ...
  function runFunnel(f) {
    var names = { licences: 'licences read', inArea: 'in the area', active: 'active', picked: 'picked', drafted: 'letters drafted', mock: 'mock letters', sent: 'sent' };
  ...
  function runCard(r, link) {
    var inner = '<div class="name" style="font-weight:600">' + esc(r.label) + (r.mock ? ' <span class="tag">mock</span>' : '') + '</div>' +
...
<script>${SCRIPT.replace('__COLOUR__', JSON.stringify(STATUS_COLOUR)).replace('__DEFAULT_MAP_AREA__', JSON.stringify(DEFAULT_MAP_AREA)).replace('__MAPS_JS__', MAPS_JS).replace('__MAP_FIT__', () => mapFit.toString())}</script>
</body>
</html>`;
}

module.exports = { pageHtml, STATUS_COLOUR, DEFAULT_MAP_AREA };

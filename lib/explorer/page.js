...
(function () {
  ...
  var boot = null, map = null, mapLayer = null, listTimer = null;
  ...
  function mapView(q) {
    ...
    if (!window.L) { document.getElementById('map').innerHTML = '<p class="muted" style="padding:12px">The map library did not load.</p>'; return; }
    map = L.map('map', { zoomControl: true });
    L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', {
      maxZoom: 19, referrerPolicy: 'strict-origin-when-cross-origin',
      attribution: '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
    }).addTo(map);
    api('pins', area ? { area: area } : {}).then(function (j) {
      var pts = [];
      mapLayer = L.layerGroup().addTo(map);
      j.pins.forEach(function (p) {
        var m = L.circleMarker([p.lat, p.lng], { radius: 9, color: '#FBF8F2', weight: 2, fillColor: COLOUR[p.status] || '#8A8178', fillOpacity: 1 });
        m.on('click', function () { pinCard(p); });
        m.addTo(mapLayer); pts.push([p.lat, p.lng]);
      });
      if (pts.length) map.fitBounds(pts, { padding: [24, 24], maxZoom: 13 }); else map.setView([31.5, -92.3], 7);
    });
  }
...
<script>${SCRIPT.replace('__COLOUR__', JSON.stringify(STATUS_COLOUR))}</script>
</body>
</html>`;
}

...
/* Until the carriers approve the texting campaign they drop our texts after
   Twilio has accepted them. The missed-call text stays off unless
   TEXTING_LIVE is "true" (surrounding spaces ignored; "TRUE", "1" or "yes"
   do not count). The spoken disclosure in the voice route does not depend
   on this: it plays on every call. */
...
/* The destinations this system ever writes to are the caller of the call in
   hand, OWNER_CELL, and — only when Steven replies from OWNER_CELL — a number
   the call and text log shows has contacted the business first. No route
   takes a destination from a request parameter, and this refuses anything
   that is not a plain E.164 number, so a malformed webhook cannot redirect a
   message. A refusal carries Twilio's error code, so a caller can tell
   "they replied STOP" (21610) from anything else. */
async function sendSms({ sid, token, from, to, body }) {
  ...
  if (!res.ok) {
    ...
    let code = null;
    try { code = JSON.parse(text).code || null; } catch (e) { /* not JSON */ }
    const err = new Error('Twilio refused the message (HTTP ' + res.status + ') ' + text.slice(0, 200));
    err.twilioCode = code;
    throw err;
  }
...
/* There is no database yet, so Twilio's own logs of texts and calls to the
   business number are the record of who has been in touch. Both helpers
   throw if Twilio will not answer; the reply route treats that as "do not
   send", because sending to the wrong person is worse than not sending. */

async function twilioList({ sid, token, resource, params }) {
  const url = `${TWILIO_API}/Accounts/${encodeURIComponent(sid)}/${resource}.json?` +
  ...
  const body = await res.json();
  return body[resource === 'Calls' ? 'calls' : 'messages'] || [];
}

/* What Twilio puts in From for a withheld or unknown caller. They look like
   numbers but nobody can be texted there. */
const WITHHELD = new Set(['+266696687', '+7378742833', '+2562533', '+8656696', '+86282452253']);
const reachable = n => isE164(n) && !WITHHELD.has(String(n).trim());

/* Has this number ever texted or called the business number? */
async function hasContacted({ sid, token, business, number }) {
  if (!reachable(number)) return false;
  const params = { From: number, To: business, PageSize: '1' };
  const [texts, calls] = await Promise.all([
    twilioList({ sid, token, resource: 'Messages', params }),
    twilioList({ sid, token, resource: 'Calls', params })
  ]);
  return texts.length > 0 || calls.length > 0;
}

/* The last person to text or call the business number, other than the
   owner, or null if nobody has. */
async function lastContact({ sid, token, business, owner, look = 50 }) {
  const params = { To: business, PageSize: String(look) };
  const [texts, calls] = await Promise.all([
    twilioList({ sid, token, resource: 'Messages', params }),
    twilioList({ sid, token, resource: 'Calls', params })
  ]);
  const when = s => { const t = Date.parse(s || ''); return Number.isNaN(t) ? 0 : t; };
  const seen = [
    ...texts.filter(m => m.direction === 'inbound')
      .map(m => ({ number: m.from, at: when(m.date_sent || m.date_created) })),
    ...calls.filter(c => c.direction === 'inbound')
      .map(c => ({ number: c.from, at: when(c.start_time || c.date_created) }))
  ].filter(x => reachable(x.number) && x.number !== owner && x.number !== business);
  seen.sort((a, b) => b.at - a.at);
  return seen.length ? seen[0].number : null;
}
...
module.exports = {
  ...
  hasContacted, lastContact, WITHHELD
};
